...
open class WorldType(
	open val allowedWorlds: () -> Map<out World, Message?> = Instance.allowedWorlds,
	...
	override fun parse(reader: StringReader): World {
		...
		return server.getWorld(name)?.takeIf { it in allowedWorlds() }
	...
	override fun <S> listSuggestions(
		...
		allowedWorlds().mapKeys { (world, _) -> world.name }
	...
	companion object Instance : WorldType(
		...
		notFoundExceptionType = ChatCommandExceptionType {
			TranslatableComponent("argument.dimension.invalid", it[0])
		}
	)
}